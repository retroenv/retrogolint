...
// StructLiteralMultilineRule detects single-line struct literals with multiple fields.
...
func (r *StructLiteralMultilineRule) Description() string {
	return "Struct literals with multiple fields should use multiple lines"
}
...
// Check analyzes a file for single-line struct literals with multiple fields.
func (r *StructLiteralMultilineRule) Check(fset *token.FileSet, file *ast.File) []violation.Violation {
	...
	ast.Inspect(file, func(node ast.Node) bool {
		...
		violations = append(violations, violation.Violation{
			Rule:     r.Name(),
			Message:  "struct literal with multiple fields should use multiple lines",
